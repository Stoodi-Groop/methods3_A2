...
#cubic growth curve model
growth3 <- lmer(CHI_MLU ~ Gender + Age + VISIT*Diagnosis + I(VISIT^3) + (1 + VISIT | ID), traindf, REML = F)

#- calculate performance of the model on the training data: root mean square error is a good measure. (Tip: google the function rmse())

#dropping na, which seemed to not be useful afterall 
#traindf <- traindf %>% 
#  drop_na(CHI_MLU)

#performance using root mean square error (using hydroGOF package is crucial)
#linear
...
testdf <- read.csv("clean_test_data.csv")
...
basic <- lmer(CHI_MLU~VISIT + Diagnosis + (1 + VISIT|ID), traindf, REML = F)

#define folds
k = 6
folds = createFolds(unique(traindf$ID), k = k, list = TRUE, returnTrain = FALSE)
...
i = 1
for (fold in folds) {
  train = subset (traindf, !(ID %in% fold))
  test = subset (traindf, ID %in% fold)
  model = basic
