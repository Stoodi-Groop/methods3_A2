...
pacman::p_load(readr,dplyr,stringr,lmerTest,Metrics,caret, lme4, hydroGOF)
...
#make full dataset
#remove column so they can be merged
traindf <- traindf[,-1]
...
#basic model with full dataframe
basic <- lmer(CHI_MLU~VISIT + Diagnosis + (1 + VISIT|ID), fulldf, REML = F)


#cubic growth curve model with full dataframe
...
#working models in ^2
model1 <- lmer(CHI_MLU ~ Gender + Age + Diagnosis*VISIT + I(VISIT^2) + (1 + VISIT | ID), fulldf, REML = F)
...
#working models in ^3
model2 <- lmer(CHI_MLU ~ Gender + Age + Diagnosis*VISIT + I(VISIT^3) + (1 + VISIT | ID), fulldf, REML = F)
...
#new model
model3 <- lmer(CHI_MLU ~ Gender + Diagnosis*VISIT + I(VISIT^3) + (1 + VISIT | ID), fulldf, REML = F) #OK
...
#filter for one child "Bernie", here we chose ID number 2 
...
#mean MLU for TD for each visit
mean_td <- aggregate(TD$CHI_MLU, list(TD$VISIT), mean, na.rm = T)

#difference between Bernie and average TD
