...
for (fold in folds) {
  ...
  basic_trainRMSE[i] = rmse(train$types_CHI, train$prediction)
  basic_testRMSE[i] = rmse(test$types_CHI, test$prediction)
  i = i + 1
}
...
for (fold in folds) {
  ...
  test$prediction = predict(model, test, allow.new.levels = T)
  train$prediction = predict(model, train, allow.new.level = T) 
  growth_trainRMSE[l] = rmse(train$types_CHI, train$prediction)
  growth_testRMSE[l] = rmse(test$types_CHI, test$prediction)
  l = l + 1
}
